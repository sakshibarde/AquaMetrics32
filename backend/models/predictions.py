...
def load_and_prepare_df():
    """Load DB data and convert ALL columns to numeric where possible."""
    df = read_sql("SELECT * FROM water_records")
    df['timestampDate'] = pd.to_datetime(df['timestampDate'], errors='coerce')
    df['date'] = df['timestampDate'].dt.date

    # Force all non-meta columns to numeric — fixes 'text' columns from Supabase
    meta_cols = {'stationId', 'timestamp', 'timestampDate', 'date'}
...
def create_daily_prediction_plots(output_json_dir=os.path.join(STATIC_PRED_DIR, "daily")):
    ...
    for station_id in stations:
        ...
        station_df = station_df.dropna(subset=DAILY_FEATURES)
        daily_avg  = station_df.groupby('date', as_index=False)[DAILY_FEATURES].mean().dropna()
        ...
        try:
            last_sequence_scaled = scaler.transform(daily_avg.iloc[-SEQ_LENGTH:][DAILY_FEATURES])
            X_pred = np.array([last_sequence_scaled])
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                future_pred_scaled = model.predict(X_pred)
            ...
            pred_dict = {'stationId': station_id, 'date': prediction_date}
            pred_dict.update({p: float(future_pred_inv[0][i]) for i, p in enumerate(DAILY_FEATURES)})
            all_daily_predictions.append(pred_dict)

            fig = go.Figure()
            fig.add_trace(go.Bar(x=DAILY_FEATURES, y=actual_today_inv.tolist(),
                                 name='Actual (Today)', marker_color='blue'))
            fig.add_trace(go.Bar(x=DAILY_FEATURES, y=future_pred_inv[0].tolist(),
                                 name='Predicted (Tomorrow)', marker_color='orange'))
            fig.update_layout(title=f"Station {station_id} - Daily Prediction for {prediction_date}",
                              barmode='group', xaxis_tickangle=-45)
            fig.write_json(os.path.join(output_json_dir, f"daily_pred_station_{station_id}.json"))
            print(f"✅ Saved daily plot for {station_id}")
        except Exception as e:
            print(f"🔴 Prediction error for {station_id}: {e}")
    ...
    if all_daily_predictions:
        pd.DataFrame(all_daily_predictions).to_json(
            os.path.join(STATIC_PRED_DIR, "daily_summary_predictions.json"), orient="records"
        )
...
def create_weekly_prediction_plots(
    ...
    for station_id in stations:
        ...
        station_df = df[df['stationId'] == station_id].sort_values('date')
        station_df = station_df.dropna(subset=WEEKLY_FEATURES)
        daily_avg  = station_df.groupby('date', as_index=False)[WEEKLY_FEATURES].mean().dropna()
        ...
        try:
            future_input_scaled = scaler.transform(daily_avg.iloc[-SEQ_LENGTH:][WEEKLY_FEATURES])
            predictions_scaled  = []
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                for _ in range(7):
                    ...
                    future_input_scaled = np.vstack([future_input_scaled[1:], y_pred])
            ...
            pred_dict = {'stationId': station_id,
                         'date': f"{start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}"}
            pred_dict.update({p: float(weekly_avg_pred[i]) for i, p in enumerate(WEEKLY_FEATURES)})
            all_weekly_predictions.append(pred_dict)

            pred_df = pd.DataFrame(predictions_inv, columns=WEEKLY_FEATURES).round(3)
            stats_df = pd.DataFrame({
                'Avg': pred_df.mean(), 'Min': pred_df.min(),
                'Max': pred_df.max(), 'Std': pred_df.std()
            }).T.round(3)
            pred_df.index = [
                f"{(start_date + timedelta(days=i)).strftime('%Y-%m-%d')} (Day {i+1})"
                for i in range(7)
            ]
            pd.concat([pred_df, stats_df]).to_json(
                os.path.join(output_details_dir, f"weekly_details_station_{station_id}.json"),
                orient="index"
            )

            fig = go.Figure()
            fig.add_trace(go.Scatter(x=WEEKLY_FEATURES, y=weekly_avg_pred.tolist(),
                                     name='Predicted Weekly Avg', marker_color='red',
                                     mode='lines+markers'))
            fig.update_layout(
                title=f"Station {station_id} - Predicted Weekly Average "
            ...
            print(f"🔴 Weekly prediction error for {station_id}: {e}")
    ...
    if all_weekly_predictions:
        pd.DataFrame(all_weekly_predictions).to_json(
            os.path.join(STATIC_PRED_DIR, "weekly_summary_predictions.json"), orient="records"
        )
